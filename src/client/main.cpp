...
void clearFileRecords(){
  std::ofstream fout;
  fout.open("res/savedGames/recordedCommands.json",std::ofstream::out |std::ofstream::trunc);
  fout.close();
}
bool checkEmptyRecords(){
  std::ifstream fin;
  fin.open("res/savedGames/recordedCommands.json");
  fin.seekg(0,fin.end);
    ...
    if (argc == 2) {
      if ( !strcmp(argv[1],"hello") ){
        ...
        affichagebeauTest("heuristic_AI");
            ...
            Client* client = new Client(e,hai);
            ...
            affichagebeauTest("play");
            if (checkEmptyRecords()){
              cout<< "Record file is empty, you need to play once with the record command to see a game"<<endl;
            } else{
              State* s = new State();
              Engine* e = new Engine(s);
        ...
        cout << "rollback : joue le jeu à l'envers au bout d'un certain temps" << endl;
        cout << "deep_ai : test de l'ia avancé" << endl;
